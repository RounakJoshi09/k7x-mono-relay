...
import { SSHTunnelManager } from "./ssh-tunnel.mjs";
...
class _database {
    ...
    try {
      ...
      if (lstConfigs.has("database-loadmethod"))
        this.config.loadmethod =
          lstConfigs.get("database-loadmethod") || "insert";
    ...
    return new Promise<void>(async (resolve, reject) => {
      try {
        ...
        if (this.config.ssh_tunnel && this.config.ssh_tunnel.enabled) {
          try {
            logger.logMessage(
              "Setting up SSH tunnel to %s:%d",
              this.config.ssh_tunnel.host,
              this.config.ssh_tunnel.port
            );
            ...
            logger.logMessage(
              "SSH tunnel established, connecting to localhost:%d",
              this.config.port
            );
          } catch (sshErr: any) {
        ...
        if (this.config.technology == "postgres") {
          ...
          try {
            ...
            if (errSystemMessage.startsWith("getaddrinfo ENOTFOUND"))
              ...
              errSystemMessage == "The server does not support SSL connections"
            )
              errorMessage =
                "Specified PostgreSQL Database Server does not support secure connection";
            else if (
              errSystemMessage.includes("connection is insecure (try using")
            )
              errorMessage =
          ...
          connectionPoolMysql = mysql.createPool(poolConfig);
          try {
            let connection = await connectionPoolMysql
              .promise()
              .getConnection();
            connection.release();
          } catch (connErr: any) {
          ...
          if (this.config.technology == "postgres") {
            ...
            try {
              ...
              if (connErr.code == "ECONNREFUSED")
                ...
                errorMessage = "Invalid MySQL username/password/Authentication";
              else errorMessage = connErr.message || "MySQL connection failed";
            ...
            const connection = new mssql.Connection({
            ...
            try {
              ...
              await new Promise<void>((resolveQuery, rejectQuery) => {
                connection.execSql(
                  new mssql.Request("SELECT 1", (err: any) => {
                    if (err) {
                      rejectQuery(err);
                    } else {
                      resolveQuery();
              ...
              if (err.message && err.message.includes("getaddrinfo ENOTFOUND"))
                ...
                err.message &&
                err.message.includes("Could not connect (sequence)")
              )
                errorMessage =
              ...
              else errorMessage = err.message || "SQL Server connection failed";
  ...
  convertCSV(
    ...
    for (let r = 0; r < lstLines.length; r++) {
      ...
      for (let c = 0; c < lstValues.length; c++) {
        ...
        if (doubleQuote) lstValues[c] = `"${targetFieldValue}"`;
    ...
    try {
      ...
      for (const line of lstLines) {
        ...
        for (let f = 0; f < lstFields.length; f++) {
          ...
          Object.defineProperty(objRow, fieldName.trim(), {
            enumerable: true,
            value: fieldValue,
          });
        }
  ...
  bulkLoad(
    ...
    return new Promise<number>(async (resolve, reject) => {
      ...
      try {
        ...
        if (this.config.loadmethod == "insert") {
          //INSERT query based loading
          ...
          while (lstLines.length) {
            ...
            while (
              lstLines.length &&
              sqlQuery.length + lstLines[0].length + 3 < maxQuerySize &&
              ++countBatch <= 1000
            ) {
    ...
    return new Promise<number>(async (resolve, reject) => {
      try {
        const [job] = await this.bigquery
          .dataset(this.config.schema)
          .table(targetTable)
          .load(`./csv/${targetTable}.csv`, {
            sourceFormat: "CSV",
            skipLeadingRows: 1,
  ...
  uploadAzureDataLake(lstTables: tableConfigYAML[]): Promise<void> {
    return new Promise<void>(async (resolve, reject) => {
      try {
        ...
        if (regDomain) domain = regDomain[1];
        ...
        const datalakeServiceClient =
          adls.DataLakeServiceClient.fromConnectionString(connectionString);
        const fileSystemClient =
          datalakeServiceClient.getFileSystemClient("tally");
        ...
        for (const targetTable of lstTables) {
          ...
          const fileClientCSV = directoryClient.getFileClient(
        ...
        connection.on("connect", async (connErr) => {
          if (connErr) {
            ...
            if (connErr.message.includes("getaddrinfo ENOTFOUND"))
              ...
              errorMessage =
                "Unable to make SQL Server connection to specified port";
            ...
            const executeQuery = (qry: string): Promise<queryResult> => {
              return new Promise<queryResult>((_resolve, _reject) => {
                connection.execSql(
                  new mssql.Request(qry, (queryErr, rowCount, rows) => {
                    if (queryErr) _reject(queryErr);
                    else _resolve({ rowCount: rowCount || 0, data: rows });
                  })
                );
              });
            };
      ...
      try {
        ...
        connection.on("connect", async (connErr) => {
          if (connErr) {
            ...
            let data: any[] = [];
            connection.execSql(
              new mssql.Request(sqlQuery, (queryErr, rowCount, rows) => {
                if (queryErr) {
                  reject(queryErr);
                } else {
                  data = rows;
  ...
  private dumpDataPostges(targetTable: string): Promise<number> {
    return new Promise<number>(async (resolve, reject) => {
      ...
      try {
        ...
        let ptrCopyQueryStream = pgLoadInto(
          `copy ${targetTable} from stdin csv header;`
        );
      ...
      try {
        connection.connect(async (connErr) => {
          if (connErr) {
            ...
            connection.query(
              sqlQuery,
              async (queryErr, results: any[] | any) => {
  ...
  private dumpDataMssql(
    ...
    return new Promise<number>(async (resolve, reject) => {
      ...
      try {
        ...
        if (!lstData.length) {
          //skip bulk insert if no rows are found
          return resolve(0);
        }
        ...
        connection.on("connect", (connErr) => {
          connection.on("errorMessage", (msg) => {
            ...
            logger.logError(
              `database.dumpDataMssql(${targetTable})`,
              msg.message + errorDescription
            );
            reject(msg);
          });
          if (connErr) {
            ...
            logger.logError(
              "database.dumpDataMssql()",
              errorMessage || connErr
            );
            reject(connErr);
          } else {
            const blOptions = { keepNulls: true };
            const oBulkLoad = connection.newBulkLoad(
              targetTable,
              blOptions,
  ...
  private populateDatabaseTableInfo(
    targetTable: string
    ...
    return new Promise<databaseFieldInfo[]>(async (resolve, reject) => {
      try {
        ...
        for (const row of result) {
          ...
          let fieldPrecision =
            row[4].value !== null ? parseInt(row[4].value) : 0;
